package com.example.getClosedIssues.presentation.ui.base
    ...
    protected fun getListener(): OnDataSourceLoading {
        return object : OnDataSourceLoading {
            override fun onFirstFetch() {
                Log.d("15haan00","inside onFirst fetch")
                showRecyclerLoading()
            }

            override fun onFirstFetchEndWithData() {
                Log.d("15haan00","inside onFirstFetchEndWithData")
                hideRecyclerLoading()
                hideEmptyVisibility()
            }

            override fun onFirstFetchEndWithoutData() {
                Log.d("15haan00","inside onFirstFetchEndWithoutData")
                hideRecyclerLoading()
                showEmptyVisibility()
            }

            override fun onDataLoading() {
                Log.d("15haan00","inside onDataLoading")
                showRecyclerLoading()
            }

            override fun onDataLoadingEnd() {
                Log.d("15haan00","inside onDataLoadingEnd")
                hideRecyclerLoading()
            }

            override fun onInitialError() {
                Log.d("15haan00","inside onInitialError")
                hideRecyclerLoading()
                showErrorToast()
            }

            override fun onError() {
                Log.d("15haan00","inside onError")
                hideRecyclerLoading()
                showErrorToast()
            }
        }
